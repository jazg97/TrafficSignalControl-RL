...
def set_train_path(models_path_name):
    ...
    dir_content = [entry.name for entry in os.scandir(models_path) if entry.is_dir()] #os.listdir(models_path)
    if dir_content:
        previous_versions = [int(name.split("_")[1]) for name in dir_content if '400' not in name]
